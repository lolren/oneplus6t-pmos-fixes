...
grep -Fqx 'User=greetd' "$SERVICE"
grep -Fqx 'Group=greetd' "$SERVICE"
grep -Fqx 'Environment=XDG_RUNTIME_DIR=/run/user/114' "$SERVICE"
grep -Fqx 'Environment=WAYLAND_DISPLAY=wayland-0' "$SERVICE"
grep -Fqx 'Environment=DBUS_SESSION_BUS_ADDRESS=unix:path=/run/user/114/bus' "$SERVICE"
