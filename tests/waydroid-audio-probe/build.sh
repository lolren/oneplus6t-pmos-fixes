...
mkdir -p "$output_dir"
probe_tmp=$(mktemp -d "${TMPDIR:-/tmp}/waydroid-audio-probe.XXXXXX")
...
javac --release 8 \
	-cp "$android_jar" \
	-d "$probe_tmp/classes" \
	"$probe_dir/src/dev/lolren/waydroidaudioprobe/AudioProbeActivity.java"
...
"$build_tools/apksigner" sign \
	...
	--out "$probe_tmp/waydroid-audio-probe.apk" \
	"$probe_tmp/probe-aligned.apk"
"$build_tools/apksigner" verify --verbose \
	"$probe_tmp/waydroid-audio-probe.apk"

install -m 0644 "$probe_tmp/waydroid-audio-probe.apk" \
	"$output_dir/waydroid-audio-probe.apk"
sha256sum "$output_dir/waydroid-audio-probe.apk"
