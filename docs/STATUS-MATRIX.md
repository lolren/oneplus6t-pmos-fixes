...
| Location | Read-only ModemManager/GeoClue report; fresh-UTC-gated polling for raw/NMEA, gpsd and formatted-ModemManager fixes; reversible Android app-op/provider bridge tied to Waydroid lifecycle; coordinate-free applied logs | A freshly restarted one-second GNSS stream has advancing UTC, positive GGA quality, active RMC and a private fix within 5 km of Stroud. Android location is enabled, fused mock-provider injections report 3.5 m estimated accuracy and its private fix is also within 5 km. Lifecycle/app-op restoration and 12 bridge tests pass | Install and accept a map app; Qualcomm A-GPS modes still fail at the operation-mode indication and the bridge remains a mock provider rather than a GNSS HAL |
