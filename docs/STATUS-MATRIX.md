...
| Android camera enumeration | Complete clean-image legacy provider plus open Camera3 HAL for all three sensors, YUV/JPEG/private streams, AF/EV metadata, mixed RGB/NV12 coalescing, private-preview cap, source-fence-safe post-processing, worker-lifecycle drain candidate and guarded main/front recording profiles | r52 is installed on a clean ARM64 Vanilla image; Android reports the provider running and the corrected Camera2 probe completed two consecutive sequential ID 0/1/2 preview runs with `valid=3 total=3`, waiting for each asynchronous camera close; no new kernel/provider fatal appeared. Installer, patch-integrity and source-compile fixtures pass | Rebuild/install the `0018` provider bundle and repeat sequential and ordinary-app open/close tests; complete all-camera full/JPEG visual acceptance; keep auxiliary recording unadvertised until a non-Venus path or kernel/Codec2 fix passes teardown |
