...
| Native camera sensors | IMX371/IMX376/IMX519 modes, Quad-Bayer handling, conservative AE, image controls, stable rear AF, normalized rear manual focus and standard automatic/manual white balance; the simple IPA exposes `LensPosition` 0.0–2.0 plus `AwbEnable`/`ColourGains` | Full pMOS host suite, deterministic AF regression, clean AArch64 r29 build, signed runtime/IPA verification, live 0.0/1.0/2.0 actuator requests, 60-second two-rear stability pass and manual/AWB round trips on all three sensors | Compare saved rear photographs against a well-lit target/chart; factory dioptre calibration, vendor CCM/lens shading and Android computational processing remain open |
| Native camera UI | Advanced Snapshot r30 source with a labelled always-visible **Image Controls** button, Gamma, automatic/manual white balance, per-sensor calibration profiles, tap-focus reticle, rear manual focus, automatic/manual exposure, shutter/gain, exposure/colour/contrast/detail controls, 1x–4x pinch/slider zoom, latest-frame preview scheduling, save-failure feedback, guarded rear hardware-flash switch, opt-in software HDR and serialized camerabin teardown | Source commit `275999b` and the verified r30 archive build; exact APK pair is installed without reboot; controls/calibration UI opened; all three sensors accepted extreme gain changes and automatic restoration; native focus helper regression passes | Verify saved chart images, playable video, HDR output, rear LED restoration and scene-specific calibration values; factory CCM/lens shading remain open |
...
| Android camera enumeration | Complete clean-image legacy provider plus open Camera3 HAL for all three sensors, YUV/JPEG/private streams, AF/EV metadata, mixed RGB/NV12 coalescing, private-preview cap, source-fence-safe post-processing, worker-lifecycle drain, rear manual focus and guarded main/front recording profiles | Current provider map is ID 0 rear main, ID 1 rear auxiliary and ID 2 front; r53-static10-focus is installed; manual-focus reports result distances 0.000–2.000 on both rear IDs, tap-focus reports terminal states and non-empty regions, and all three cameras return valid streams; installer, patch-integrity and source-compile fixtures pass | Soak an ordinary Android camera application across open/close and app switching; keep auxiliary ID 1 recording unadvertised until a non-Venus path or kernel/Codec2 fix passes teardown |
