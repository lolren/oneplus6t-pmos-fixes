...
| Native camera UI | Advanced Snapshot r16 source with tap-focus reticle, rear manual focus, automatic/manual exposure, shutter/gain, exposure/colour/contrast/detail controls, 1x–4x pinch/slider zoom, latest-frame preview scheduling, save-failure feedback, guarded rear hardware-flash switch, opt-in software HDR with bounded handheld translation alignment and serialized camerabin teardown | Pinned GTK/GStreamer build and formatting pass; commit `51139b2` built as AArch64 r16 and installed without reboot; native focus helper regression passes | Verify visual preview latency, saved images, manual exposure, playable video, touchscreen tap/manual-focus interaction, HDR output and rear LED restoration on the phone |
