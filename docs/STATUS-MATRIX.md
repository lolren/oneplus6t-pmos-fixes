...
| Mobile data | NetworkManager profile helper; provider database/GID-aware MVNO selection; UUID-scoped stale-bearer watchdog; managed rollback; guarded native/Waydroid cellular-only acceptance command with exact Wi-Fi restoration | Earlier live SMARTY/3 UK LTE, Wi-Fi-off DNS/HTTPS, Waydroid NAT and explicit network-side bearer-loss/watchdog recovery pass; fixtures cover health, cooldown, call deferral, resolver fallback, route failure, privilege preflight and Wi-Fi restoration failures; signed runtime r20 is byte-reproducible and passes AArch64 install simulation | Recover normal SSH, install exact r20 and run the new unattended test against the newly inserted SIM after a cold boot; test an additional non-SMARTY SIM to broaden carrier acceptance |
...
| Android camera enumeration | Open Camera3 HAL for all three sensors, YUV/JPEG/private streams, AF/EV metadata, mixed RGB/NV12 coalescing, private-preview cap and per-camera recording profiles | All three cameras pass single-output probes; installed r44 configures mixed preview+encoder and CameraX sees the declared 480p/720p profiles | Repeat the complete three-camera probe after recovery and test front/auxiliary recording in real applications |
| Android camera performance/video | GPU software ISP with NV12 colour correction, coalesced multi-output processing, ranked Venus Codec2 H.264 service, MMAP compressed-output bridge and bounded minijail policy | Live r50 hardware encode finalized 460 rear frames over 25.556 s at exactly 18 fps with 48 kHz mono AAC; two clean r51 builds and packages are byte-identical | Install the exact reproducible r51 archive after physical recovery, test an illuminated rear scene plus front/auxiliary video, then benchmark long capture and suspend/resume |
...
| Waydroid apps | Guarded camera/Codec2 overlay installers, health preflight and separate GAPPS verifier | Installer, mount/I/O-pressure, reproducible-build/package and GAPPS fixture tests pass | A foreground-session interruption left Android init zombie and one D-state process; physically reboot, require a clean unmounted rootfs, then install/test r51 |
...
USB: CDC-NCM networking; 18d1:d001 is the pMOS composite gadget, not bootloader fastboot
Fastboot: no device
OnePlus ADB: no device
SSH: authentication succeeds, but a new session does not open while teardown is wedged
Waydroid: init zombie plus one D-state Android process after foreground-session interruption
```

Do not flash a partition, boot slot or firmware. A physical power-button reboot
is required before further phone mutation; after it, require normal SSH, no
Waydroid rootfs mounts and zero PSI I/O pressure. The installer deliberately
refuses to bypass this gate.
