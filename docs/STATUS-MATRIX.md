...
| Native camera UI | Advanced Snapshot r13 source with tap-focus reticle, automatic/manual exposure, shutter/gain, exposure/colour/contrast/detail controls, 1x–4x pinch/slider zoom, latest-frame preview scheduling, save-failure feedback and guarded rear hardware-flash switch | C helper syntax, Rust formatting, cross-compiled 6+9 test suite and signed AArch64 r13 package validator pass; phone acceptance pending | Install the r13 app only after the matching libcamera generation is built and validated, then verify visual preview latency, saved images, manual exposure, playable video and rear LED restoration on the phone |
