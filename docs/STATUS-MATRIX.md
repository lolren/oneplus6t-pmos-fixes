...
| Mobile data | NetworkManager profile helper; provider database lookup; GID1-aware MVNO overlay; explicit APN fallback; managed rollback | APN tests pass; live SMARTY/3 UK LTE bearer, network-supplied address/DNS and QMAP-forced traffic pass | Repeat after a cold boot and test an additional non-SMARTY SIM to broaden carrier acceptance |
