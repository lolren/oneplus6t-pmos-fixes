...
| Native camera UI | Advanced Snapshot r38 source with a labelled always-visible **Image Controls** button, phone-width colour calibration, Gamma, automatic/manual white balance, per-sensor matrix profiles, visible green-cast correction action, tap-focus reticle, rear manual focus, automatic/manual exposure, shutter/gain, exposure/colour/contrast/detail controls, unobstructed toolbar 1x–4x pinch/slider zoom, latest-frame preview scheduling, save-failure feedback, guarded rear hardware-flash switch, opt-in software HDR and serialized camerabin teardown; r38 adds a fresh centre-weighted autofocus request on the standalone still stream when no tap or manual lock is selected | Source commit `5e102b7` and the verified r38 AArch64 package build/validator; local runtime r49 installs the checksum/signature-verified r38 pair; the 1.0× chip and calibration dialog passed full-resolution screen inspection; all three sensors accepted identity/custom matrix requests and automatic restoration; native focus helper regression, live r35 smoke and package tests pass | Verify saved chart images, playable video, HDR output, rear LED restoration, Green-cast Apply/Reset behavior and measured scene-specific calibration values; factory CCM/lens shading remain open |
...
| Android camera enumeration | Complete clean-image legacy provider plus open Camera3 HAL for all three sensors, YUV/JPEG/private streams, AF/EV metadata, mixed RGB/NV12 coalescing, private-preview cap, source-fence-safe post-processing, worker-lifecycle drain, rear manual focus and guarded main/front recording profiles | Current provider map is ID 0 rear main, ID 1 fixed-focus front and ID 2 rear auxiliary; r53-static10-focus is installed; the post-sync preview returned valid private frames on all three IDs, while the earlier full probe returned valid YUV/JPEG/private previews with zero JPEG row discontinuities; rear AF states `[3,4]` and centre regions remain on IDs 0/2, fixed-focus state `[0]` remains on ID 1, and the matching manual-focus probe returned a 0→2 dioptre delta on IDs 0/2; r35 also handles denied sleep inhibition and the root-only shell boundary; the r47 profile correction retained by r48, installer, patch-integrity and source-compile fixtures pass | Soak an ordinary Android camera application across open/close and app switching; keep auxiliary ID 2 ordinary recording unadvertised until a non-Venus path or kernel/Codec2 fix passes teardown |
| Android camera performance/video | GPU software ISP with NV12 colour correction, coalesced multi-output processing, ranked Venus Codec2, MMAP compressed output, exact single-plane NV12 layout/lifetime and metadata-only temporary stride discovery | The local r49 runtime is installed and retains the r47 stopped-rootfs synchronizer, which changed both profile files to source SHA-256 `1ba320e0f74225d41beb6bf8ffc0c75ae6102938d230245b0f472629a0e51dfe`; Android reports ordinary 480p/720p profiles on IDs 0/1 and no ordinary profile on ID 2; real 720p H.264/AAC recordings pass on rear-main ID 0 (176 frames) and front ID 1 (248 frames), while the ID-2 encoder remains guarded after the Venus teardown fault | Continue long capture, app switching and suspend/resume only on IDs 0 and 1; Android camera-app acceptance and Android-vendor performance/image parity remain open |
...
| Waydroid apps | Pinned Google-free Vanilla image pair, read-only no-Google verifier, guarded camera/Codec2 installers, health preflight, streamed Camera2 probe, persistent PipeWire playback bridge safeguards and a disabled persistent graphical-session unit | Android 13 Vanilla is installed; GMS, GSF, Play Store and the GMS property are absent. r53-static10-focus camera and r53 Codec2 overlays are present; local runtime r49 is installed, the profile synchronizer has applied its guarded mapping, and post-sync preview plus main/front H.264/AAC probes pass; the idle container remains freeze-on-idle outside probe invocations; the corrected session unit is installed but remains disabled until a normal graphical user session exists, so Waydroid is not started on the greeter | Enable the corrected graphical-session unit after logging into the normal graphical account, then validate ordinary applications and map-location behavior; never encode auxiliary rear ID 2 in Venus |
