...
| Display/kernel safety | Read-only DRM/backlight diagnostic; serialized Samsung brightness writes; r10 bounds Venus recovery IRQ work only during firmware error; r8 rollback manifest | r10 signed APK is installed and booted; three guarded Venus recordings pass with no fault/IRQ storm, and manager tests cover normal-user apply plus root-safe repository verification | Capture before/after display reports and complete repeated slider, lock/unlock and suspend/resume acceptance; healthy codec runs do not exercise the r10 error branch |
...
| Android camera performance/video | GPU software ISP with NV12 colour correction, coalesced multi-output processing, ranked Venus Codec2, MMAP compressed output, exact single-plane NV12 layout/lifetime and metadata-only temporary stride discovery | r53 builds/packages are byte-identical and are installed. Three rear Aperture cycles—including cold relaunch—reach keyframe/finalize with clean teardown. A 23.84 s illuminated H.264/AAC file decodes fully without the former green band, but averages only 11.62 fps | Profile and improve the current frame rate; then accept front/auxiliary video, long capture, app switching and suspend/resume |
...
| Waydroid apps | Guarded camera/Codec2 overlay installers, health preflight, streamed Camera2 probe installation and separate GAPPS verifier | r53 acceptance ends with the container inactive, zero Waydroid mounts, zero D-state tasks and restored greeter permissions; installer, build/package, probe and GAPPS fixture tests pass | Repeat complete all-camera probes and validate Play Store application lifecycle after camera performance work |
...
Waydroid: r53 installed; container inactive; rootfs mounts 0; D-state tasks 0
Cellular: registered/connected; cellular-only DNS and HTTPS pass; Wi-Fi restored
```

No physical recovery gate remains. Continue to require normal SSH, no Waydroid
rootfs mounts and zero PSI I/O pressure before every overlay operation. Do not
