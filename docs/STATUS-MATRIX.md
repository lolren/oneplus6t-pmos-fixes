...
| Native camera UI | Advanced Snapshot r13 source with tap-focus reticle, automatic/manual exposure, shutter/gain, exposure/colour/contrast/detail controls, 1x–4x pinch/slider zoom, latest-frame preview scheduling, save-failure feedback and guarded rear hardware-flash switch | C helper syntax, Rust formatting and AArch64 package build pending; phone acceptance pending | Build/install the r13 app source, then verify visual preview latency, saved images, manual exposure, playable video and rear LED restoration on the phone |
