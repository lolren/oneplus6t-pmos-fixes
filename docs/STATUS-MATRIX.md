...
| Waydroid apps | Pinned Google-free Vanilla image pair, read-only no-Google verifier, guarded camera/Codec2 installers, health preflight and streamed Camera2 probe | Android 13 Vanilla is installed; GMS, GSF, Play Store and the GMS property are absent. r52 camera and r53 Codec2 overlays are present; all three preview/surface probes, Android networking, native LTE HTTPS, audio services and post-suspend ID-0 smoke tests pass; the idle container freezes | Validate ordinary applications, safe IDs 0/1 recording and map-location behavior after the image replacement; never encode camera ID 2 in Venus |
