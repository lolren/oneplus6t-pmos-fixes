...
| Native camera sensors | IMX371/IMX376/IMX519 modes, Quad-Bayer handling, conservative AE, image controls, stable rear AF and normalized rear manual focus; the simple IPA exposes `LensPosition` 0.0–2.0 and maps it to the safe 400–800 actuator span | Full pMOS host suite, deterministic AF regression, clean AArch64 r28 build, signed runtime/IPA verification, live 0.0/1.0/2.0 actuator requests and 60-second two-rear stability pass | Compare saved rear photographs against a well-lit target/chart; factory dioptre calibration, vendor CCM/lens shading and Android computational processing remain open |
| Native camera UI | Advanced Snapshot r16 source with a labelled always-visible **Controls** button, hamburger fallback, tap-focus reticle, rear manual focus, automatic/manual exposure, shutter/gain, exposure/colour/contrast/detail controls, 1x–4x pinch/slider zoom, latest-frame preview scheduling, save-failure feedback, guarded rear hardware-flash switch, opt-in software HDR with bounded handheld translation alignment and serialized camerabin teardown | Pinned GTK/GStreamer build and formatting pass; commit `2d9639b` built as AArch64 r16; native focus helper regression passes; phone installation is pending SSH-service recovery | Install and verify the new UI on the phone, then verify visual preview latency, saved images, manual exposure, playable video, touchscreen tap/manual-focus interaction, HDR output and rear LED restoration |
...
| Android camera enumeration | Complete clean-image legacy provider plus open Camera3 HAL for all three sensors, YUV/JPEG/private streams, AF/EV metadata, mixed RGB/NV12 coalescing, private-preview cap, source-fence-safe post-processing, worker-lifecycle drain, rear manual focus and guarded main/front recording profiles | r53-static10-focus is installed; manual-focus reports result distances 0.000–2.000 on both rear IDs, tap-focus reports terminal states and non-empty regions, and all three cameras return valid streams; installer, patch-integrity and source-compile fixtures pass | Soak an ordinary Android camera application across open/close and app switching; keep auxiliary recording unadvertised until a non-Venus path or kernel/Codec2 fix passes teardown |
| Android camera performance/video | GPU software ISP with NV12 colour correction, coalesced multi-output processing, ranked Venus Codec2, MMAP compressed output, exact single-plane NV12 layout/lifetime and metadata-only temporary stride discovery | Main rear H.264/AAC fully decodes at 11.78 fps; a dedicated safe control reaches 29.78 fps. Front r50 video fully decodes at 24.77 fps with AAC. Both auxiliary teardown orders trigger the same Venus IRQ storm, so the installed r53-static9 camera overlay retains the r51 no-profile safety policy and the probe hard-refuses it | Investigate an explicit software encoder or kernel/Codec2 fix; continue long capture, app switching and suspend/resume only on main/front |
