...
| Android camera enumeration | Complete clean-image legacy provider plus open Camera3 HAL for all three sensors, YUV/JPEG/private streams, AF/EV metadata, mixed RGB/NV12 coalescing, private-preview cap, source-fence-safe post-processing and guarded main/front recording profiles | r52 is installed on a clean ARM64 Vanilla image; Android reports the provider running and all three isolated preview/surface probes pass with no kernel/provider fatal. Installer fixture proves provider/property install and exact rollback | Complete all-camera full/JPEG visual acceptance; keep auxiliary recording unadvertised until a non-Venus path or kernel/Codec2 fix passes teardown |
...
| Waydroid apps | Pinned Google-free Vanilla image pair, read-only no-Google verifier, guarded camera/Codec2 installers, health preflight and streamed Camera2 probe | Android 13 Vanilla is installed; GMS, GSF, Play Store and the GMS property are absent. r52 camera and r53 Codec2 overlays are present, all three preview/surface probes pass, and the idle container freezes | Validate ordinary applications, safe IDs 0/1 recording, audio, cellular and location after the image replacement; never encode camera ID 2 in Venus |
