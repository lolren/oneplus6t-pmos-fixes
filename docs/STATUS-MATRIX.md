...
| Android camera enumeration | Complete clean-image legacy provider plus open Camera3 HAL for all three sensors, YUV/JPEG/private streams, AF/EV metadata, mixed RGB/NV12 coalescing, private-preview cap, source-fence-safe post-processing, worker-lifecycle drain, rear manual focus and guarded main/front recording profiles | Current provider map is ID 0 rear main, ID 1 fixed-focus front and ID 2 rear auxiliary; r53-static10-focus is installed; the current-source full probe returned valid YUV/JPEG/private previews on all three IDs with zero JPEG row discontinuities, rear AF states `[3,4]` on IDs 0/2 and fixed-focus state `[0]` on ID 1; the matching manual-focus probe returned a 0→2 dioptre delta on IDs 0/2, and the tap-focus probe returned centre regions on IDs 0/2; r35 also handles denied sleep inhibition and the root-only shell boundary; profile correction and installer, patch-integrity and source-compile fixtures pass | Soak an ordinary Android camera application across open/close and app switching; keep auxiliary ID 2 ordinary recording unadvertised until a non-Venus path or kernel/Codec2 fix passes teardown |
| Android camera performance/video | GPU software ISP with NV12 colour correction, coalesced multi-output processing, ranked Venus Codec2, MMAP compressed output, exact single-plane NV12 layout/lifetime and metadata-only temporary stride discovery | The current protected probe reports 480p/720p profiles on IDs 0/2 from the stale image-level files, although ID 2 is the auxiliary rear module; the checked-in correction moves ordinary profiles to IDs 0/1 and retains the ID 2 sentinel. Main/front encoded acceptance remains to be rerun after synchronization; auxiliary teardown faults remain blocked | Verify the corrected profile after a stopped-rootfs sync, then run only main ID 0 and front ID 1 recording; continue long capture, app switching and suspend/resume only on IDs 0 and 1 |
...
| Waydroid apps | Pinned Google-free Vanilla image pair, read-only no-Google verifier, guarded camera/Codec2 installers, health preflight, streamed Camera2 probe, persistent PipeWire playback bridge safeguards and a disabled persistent greetd/Wayland session unit | Android 13 Vanilla is installed; GMS, GSF, Play Store and the GMS property are absent. r53-static10-focus camera and r53 Codec2 overlays are present; current-source all-camera preview/manual/tap/full probes pass; the stale profile arrangement has been corrected in source and awaits stopped-rootfs synchronization; the idle container remains freeze-on-idle outside probe invocations; the new service is source/installation-tested but not enabled on the phone | Synchronize and verify the corrected profiles, then enable the graphical-session unit locally and validate ordinary applications and map-location behavior; never encode auxiliary rear ID 2 in Venus |
