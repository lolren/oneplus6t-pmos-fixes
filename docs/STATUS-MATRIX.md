...
| Native camera UI | Advanced Snapshot r24 source with a labelled always-visible **Image Controls** button in the preview overlay, Gamma, per-sensor calibration profiles, hamburger fallback, tap-focus reticle, rear manual focus, automatic/manual exposure, shutter/gain, exposure/colour/contrast/detail controls, 1x–4x pinch/slider zoom, latest-frame preview scheduling, save-failure feedback, guarded rear hardware-flash switch, opt-in software HDR with bounded handheld translation alignment and serialized camerabin teardown | Source commit `1b7b6e6` builds from the verified archive; r24 APK pair is installed without reboot; controls panel and calibration dialog opened and profile persistence/clear verified; native focus helper regression passes | Verify visual preview latency, saved images, manual exposure, playable video, touchscreen tap/manual-focus interaction, HDR output, rear LED restoration and scene-specific calibration values |
