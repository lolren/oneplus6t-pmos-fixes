...
| Native camera sensors | IMX371/IMX376/IMX519 modes, Quad-Bayer handling, conservative AE, image controls and stable rear AF | Source/package checks and prior bounded all-sensor acceptance are recorded | Reinstall the exact generation and repeat live preview, still, focus, exposure, colour and suspend/resume tests |
| Native camera UI | Advanced Snapshot package with tap-focus reticle, exposure/colour/contrast/detail controls, 1x–4x pinch/slider zoom and latest-frame preview scheduling | Clean GTK/Aperture tests, package checks and gesture trace pass | Verify visual preview latency, saved images and playable video on the phone |
...
| Location | Read-only ModemManager/GeoClue report and dry-run-first Android test-provider bridge | Four bridge tests, Python compilation and location report tests pass | Obtain a genuine native GNSS fix, validate GeoClue, then inject only accepted coordinates into Waydroid; the bridge is not a GNSS HAL |
