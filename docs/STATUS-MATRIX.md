...
| Android camera enumeration | Complete clean-image legacy provider plus open Camera3 HAL for all three sensors, YUV/JPEG/private streams, AF/EV metadata, mixed RGB/NV12 coalescing, private-preview cap, source-fence-safe post-processing, worker-lifecycle drain and guarded main/front recording profiles | r53-static9 is installed on a clean ARM64 Vanilla image; five ID-0 preview reopen cycles, two rounds across IDs 0/1/2 and one full YUV/JPEG/private probe per ID returned valid frames; `dumpsys media.camera` ended with no in-flight requests. Installer, patch-integrity and source-compile fixtures pass | Soak an ordinary Android camera application across open/close and app switching; keep auxiliary recording unadvertised until a non-Venus path or kernel/Codec2 fix passes teardown |
