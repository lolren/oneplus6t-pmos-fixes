...
| Native camera UI | Advanced Snapshot r11 source with tap-focus reticle, automatic/manual exposure, shutter/gain, exposure/colour/contrast/detail controls, 1x–4x pinch/slider zoom, latest-frame preview scheduling, save-failure feedback and guarded rear hardware-flash switch | C helper syntax, Rust formatting and source wiring pass; full GTK package build and phone acceptance pending | Build/install the new app source, then verify visual preview latency, saved images, manual exposure, playable video and rear LED restoration on the phone |
